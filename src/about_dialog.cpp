...
AboutDialog::AboutDialog(Gtk::Window& parent)
    : Gtk::Dialog("About Ephemeris", parent, true)
{
  set_resizable(false);
  ...
  title->set_markup("<b>Ephemeris " VERSION "</b>");
  box->pack_start(*title, Gtk::PACK_SHRINK);
  auto* line = Gtk::manage(new Gtk::Label(
      "Ephemeris — a paper appointment book for The Lunduke Computer Operating System."));
  ...
  box->pack_start(*Gtk::manage(new Gtk::Label("Released under The Unlicense.")),
                  Gtk::PACK_SHRINK);
  show_all_children();
}
