#!/bin/sh
# Build release artifacts: tarball, .deb, AppImage.
...
do_tarball() {
  ...
  src="${ROOT}/build/meson-dist/ephemeris-${VERSION}.tar.xz"
  if [ -f "$src" ]; then
    cp -f "$src" "$DISTDIR/"
    echo "tarball: ${DISTDIR}/ephemeris-${VERSION}.tar.xz"
  else
    echo "meson dist did not produce ephemeris-${VERSION}.tar.xz" >&2
...
do_deb() {
  ...
  mkdir -p "$DISTDIR"
  for f in "${ROOT}/../ephemeris_${VERSION}"-*.deb \
           "${ROOT}/../ephemeris-dbgsym_${VERSION}"-*.deb; do
    [ -e "$f" ] || continue
    mv -f "$f" "$DISTDIR/"
    echo "deb: $DISTDIR/$(basename "$f")"
  done
  for f in "${ROOT}/../ephemeris_${VERSION}"-*.buildinfo \
           "${ROOT}/../ephemeris_${VERSION}"-*.changes; do
    [ -e "$f" ] || continue
    mv -f "$f" "$DISTDIR/"
  done
}
...
do_appimage() {
  ...
  for f in "${ROOT}/Ephemeris-${VERSION}"-*.AppImage \
           "${ROOT}/ephemeris-${VERSION}"-*.AppImage \
           "${ROOT}/build/Ephemeris-${VERSION}"-*.AppImage \
           "${ROOT}"/*.AppImage; do
    [ -e "$f" ] || continue
    mv -f "$f" "$DISTDIR/"
  done
